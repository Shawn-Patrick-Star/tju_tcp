...
#define SYN_FLAG_MASK 0x8 // 1000
#define ACK_FLAG_MASK 0x4 // 0100
#define FIN_FLAG_MASK 0x2 // 0010
...
typedef struct {
    ...
    uint8_t ext;				//1 byte  padding 一些额外的数据 在这个项目里是为了将header的大小凑整到20bytes 没有实际意义
} tju_header_t;
...
/**
 * @brief 输入header所有字段 和 TCP包数据内容及其长度 构造tju_packet_t
 * @return 返回其指针 
 */
tju_packet_t* create_packet(
	uint16_t src, 
	uint16_t dst, 
	uint32_t seq, 
    uint32_t ack,
...
void send_packet_wrapper(tju_packet_t* pkt);
...
void printPacket(tju_packet_t* packet);
