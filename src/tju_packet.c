...
void send_packet_wrapper(tju_packet_t *pkt){    
    ...
    sendToLayer3(msg, pkt->header.plen);
    free(msg); msg = NULL;
}
...
/**
 * @brief 输入一个tju_packet_t的指针
 * @return 返回其对应的字符串
 */
char* packet_to_buf(tju_packet_t* p){
    char* msg = header_in_char(
        p->header.source_port, p->header.destination_port, 
        p->header.seq_num, p->header.ack_num, p->header.hlen, p->header.plen, 
        p->header.flags, p->header.advertised_window, p->header.ext);
    
    if(p->header.plen > p->header.hlen){ // 如果有数据
